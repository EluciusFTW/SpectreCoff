...
type PromptExample() =
    ...
    override _.Execute(_context, _, _) =
        ...
        P $"Starting you off on {chosenFruit} there, you settled on {favourite}"
        |> toConsole

        match chosenFruits with
        | [] -> "You don't like any fruit??"
        | [ single ] ->
            if (chosenFruit = single) then
                "Makes sense"
            else
                $"Why didn't you pick {chosenFruit} in the first place?"
        | _ -> $"Must be nice to like {chosenFruits.Length} different fruit!"
        |> printMarkedUp

        let amount = $"How many {chosenFruit} do you want?" |> ask<int>
        ...
        let amountOnceMore =
            ...
            |> askWithSuggesting<int>
                {
                    defaultOptions with
                        EditableSuggestion = true
                }
                16

        P $"You settled on {amountOnceMore} in the end" |> toConsole

        (if (amount = amountAgain) then
        ...
        let allowNoChoiceOptions = {
            defaultGroupedSelectionOptions with
                Optional = true
        }

        let stronglyTypedResult =
            chooseGroupedFromWith
                allowNoChoiceOptions
                stronglyTypedFoods
                "Choose to measure the tastiness and healthiness of your foods"
        ...
        | _ ->
            let healthiness =
                stronglyTypedResult
                |> List.fold (fun acc food -> acc + food.Healthiness.Value) 0

            let tastiness = stronglyTypedResult |> List.fold (fun acc food -> acc + food.Tastiness.Value) 0

            P $"The combined healthiness of your foods is {healthiness} and the combined tastiness is {tastiness}"
            |> toConsole

        let stringlyTypedFoods = {
            defaultChoiceGroups with
                Groups = [
                    {
                        Group = "Fuits"
        ...
        P $"Aha, so combined you like {stringlyTypedResult.Length} kinds of fruits and berries"
        |> toConsole
