...
let withDescriptionColumn template = {
    template with
        Columns = template.Columns @ [ TaskDescriptionColumn() ]
}

let withRemainingTimeColumn template = {
    template with
        Columns = template.Columns @ [ RemainingTimeColumn() ]
}

let withSpinnerColumn template = {
    template with
        Columns = template.Columns @ [ SpinnerColumn() ]
}

let withProgressBarColumn template = {
    template with
        Columns = template.Columns @ [ ProgressBarColumn() ]
}

let withPercentageColumn template = {
    template with
        Columns = template.Columns @ [ PercentageColumn() ]
}
...
let startCustom template (operation: ProgressOperation<'T>) =
    task { return! AnsiConsole.Progress().Columns(template.Columns |> List.toArray).StartAsync(operation) }

let start<'a> : ProgressOperation<'a> -> Task<'a> = startCustom defaultTemplate
