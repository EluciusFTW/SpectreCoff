...
let private figletTests name tests = testList name tests |> testSequencedGroup "figlet"

[<Tests>]
let layoutModeTests =
  figletTests "figlet layout mode" [
    test "is FullSize out of the box" {
      defaultLayoutMode |> should equal FigletLayoutMode.FullSize
    }

    test "figlet takes the mode from the default" {
      withDefaults FigletLayoutMode.Smushed defaultAlignment (fun () ->
        figlet "Quince" |> asFigletText |> fun f -> f.LayoutMode |> should equal FigletLayoutMode.Smushed)
    }
  ...
  figletTests "figlet styling" [
    ...
    test "figlet takes the alignment from the default" {
      withDefaults defaultLayoutMode Left (fun () ->
        figlet "Quince" |> asFigletText |> fun f -> f.Justification |> should equal (System.Nullable Justify.Left))
    }
  ]
