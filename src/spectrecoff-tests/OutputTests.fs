...
let markupTests =
    testList "markup" [
        ...
        test "non-empty style wraps content in markup tags" {
            "hello" |> markup "bold" |> should equal "[bold]hello[/]"
        }

        test "content with markup characters is escaped" {
            "he[llo" |> markup "bold" |> should equal "[bold]he[[llo[/]"
        }
    ]
...
let toMarkedUpStringTests =
    testList "toMarkedUpString" [
        ...
        test "Emoji without colons gets wrapped in colons" {
